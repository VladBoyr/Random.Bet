...
internal class Program
{
    ...
    private static async Task Main(string[] args)
    {
        ...
        await page.GotoAsync(matchList, new() { Timeout = 60000 });
    ...
    private static async Task GetMatch(string matchLink)
    {
        ...
        await page.GotoAsync(matchLink, new() { Timeout = 60000 });
        ...
        Console.WriteLine($"{await page.Locator(team1Path).InnerTextAsync(new() { Timeout = 60000 })} - " +
                          $"{await page.Locator(team2Path).InnerTextAsync(new() { Timeout = 60000 })} , " +
    ...
    {
        var arr = (await scoreLocator.InnerTextAsync(new() { Timeout = 60000 })).Split("–");
        return (int.Parse(arr[0].Trim()), int.Parse(arr[1].Trim()));
    }

    private static async Task<decimal> GetMatchKoef()
    {
        if (decimal.TryParse(await koefLocator.InnerTextAsync(new() { Timeout = 60000 }), CultureInfo.InvariantCulture, out decimal result))
            return result;
        return -1;
    }
    ...
    private static async Task<bool> SetTeam1Score(int newScore)
    {
        ...
        while (currentScore != newScore)
        {
            if (currentScore > newScore)
            {
                ...
                await team1btn1.ClickAsync(new() { Timeout = 60000 });
            }
            else
            {
                ...
                await team1btn2.ClickAsync(new() { Timeout = 60000 });
            }
    ...
    private static async Task<bool> SetTeam2Score(int newScore)
    {
        ...
        while (currentScore != newScore)
        {
            if (currentScore > newScore)
            {
                ...
                await team2btn1.ClickAsync(new() { Timeout = 60000 });
            }
            else
            {
                ...
                await team2btn2.ClickAsync(new() { Timeout = 60000 });
            }
